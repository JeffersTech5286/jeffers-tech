...
function generateIcons(
    ...
    const instances = baseIcons.flatMap((icon) => {
...
export default function FloatingIcons(): ReactNode {
    ...
    const iconsRef = useRef<RefObject<HTMLElement | null>[]>([]);
    ...
    useEffect(() => {
        ...
        function detectAndResolveCollisions(ps: Particle[]) {
    ...
    return (
        <div
            ref={containerRef}
            className="absolute inset-0 w-full h-full -z-10 bg-cls-bg"
        ...
        >
            {iconsRef.current.map((ref, i) => (
                <i
                    ...
                    ref={ref}
                    className={`${
                        baseIcons[i % baseIcons.length]
                    } text-gray-400 pointer-events-none`}
                    style={{
                        fontSize: `${
                            particles.current[i]?.size || 40
                        }px`,
                        position: "absolute",
                        top: 0,
                        left: 0,
